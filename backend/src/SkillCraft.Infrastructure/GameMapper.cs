...
internal class GameMapper
{
  ...
  public GameMapper(IReadOnlyDictionary<ActorId, Actor> actors)
  {
  ...
  {
    WorldModel destination = new(source.Slug, source.Name)
    {
      Id = source.Id,
  ...
  private void MapAggregate(AggregateEntity source, AggregateModel destination)
  {
    ...
    destination.CreatedBy = FindActor(source.UpdatedBy);
    destination.CreatedOn = source.UpdatedOn.AsUniversalTime();
  }
