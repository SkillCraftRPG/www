...
- `ADMIN_TITLE`: the title of the API, used for `/api` route and Swagger UI. Defaults to `SkillCraft API`.
...
- `MONGOCONNSTR_SkillCraft`: the MongoDB server connection string. This is currently only used for logging.
- `MONGODB_DATABASE_NAME`: the MongoDB database name. This is currently only used for logging. Defaults to `skillcraft`.
...
- `POSTGRESQLCONNSTR_SkillCraft`: the PostgreSQL connection string.
